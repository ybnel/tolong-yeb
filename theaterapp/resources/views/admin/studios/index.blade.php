...
@section('content')
    <div class="flex-1 p-8">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.studios.create') }}" class="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-xl h-10 px-4 bg-[#0c77f2] text-white text-sm font-bold leading-normal tracking-[0.015em]">
            ...
            <table class="min-w-full divide-y divide-[#111418]">
                <thead class="bg-[#111418]">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-[#9caaba] uppercase tracking-wider">Total Seats</th> {{-- UBAH: dari Capacity menjadi Total Seats --}}
                        {{-- Hapus kolom 'Seating Template' karena tidak ada di DB Anda --}}
                ...
                <tbody class="divide-y divide-[#111418]">
                    @forelse($studios as $studio)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $studio->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $studio->total_seats }}</td> {{-- UBAH: mengakses total_seats --}}
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.studios.edit', $studio->id) }}" class="text-[#0c77f2] hover:text-[#0a63d4] mr-2">Edit</a>
